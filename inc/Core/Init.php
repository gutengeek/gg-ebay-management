...
use GGEM\Admin\Admin;
use GGEM\Admin\Metabox as Metabox;
use GGEM\Common\Interfaces\Intergration;
use GGEM\Common\Module\Paypal\Paypal_Hook;
use GGEM\Common\Module\Stripe\Stripe_Hook;
use GGEM\Common\Module\Report\Report;
use GGEM\Common\Posttypes;
...
class Init {
	...
	public function __construct() {
		$this->plugin_name        = GGEM;
		$this->version            = GGEM_VERSION;
		$this->plugin_basename    = GGEM_PLUGIN_BASENAME;
		$this->plugin_text_domain = GGEM_PLUGIN_TEXT_DOMAIN;
	...
	public function define_intergrations() {
		...
		foreach ( $intergrations as $intergration ) {
			$class  = "GGEM\\Common\\Integrations\\" . $intergration;
			$object = new   $class();
	...
	private function define_global_init() {
		...
		$settings     = get_option( 'ggem_settings' );
		$ggem_options = apply_filters( 'ggem_get_settings', $settings );
	...
	public function load_admin_controller( $call ) {
		...
		$controller = "\GGEM\Admin\Controller\\" . $controller;
