...
abstract class Metabox {
	...
	public function __construct() {
		...
		add_action( 'save_post_ggems', [ $this, 'save' ], 10, 2 );
	}
	...
	public function get_needed_update_options() {
		...
		if ( ! empty( $form_meta_keys ) ) {
			foreach ( $form_meta_keys as $form_meta_key ) {
				...
				if ( 'taxonomy_select' === $setting_field['type'] ) {
					if ( isset( $setting_field['multiple'] ) && $setting_field['multiple'] ) {
						$_POST[ $form_meta_key ] = isset( $_POST[ $form_meta_key ] ) ? ggem_clean( $_POST[ $form_meta_key ] ) : [];
					} else {
						$_POST[ $form_meta_key ] = isset( $_POST[ $form_meta_key ] ) ? ggem_clean( $_POST[ $form_meta_key ] ) : '';
					}
				}
	...
	public function save_settings_options( $options, $option_name ) {
		...
		if ( $update_options ) {
			...
			do_action( 'ggem_after_save_settings', $update_options, $old_options );
		}
	}
