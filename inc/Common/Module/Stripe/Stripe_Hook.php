<?php
namespace GGEM\Common\Module\Stripe;

use GGEM\Common\Model\Query\Stripe_Query;
...
class Stripe_Hook {
	...
	public function __construct() {
		...
		if ( 'on' === ggem_get_option( 'enable_stripe', 'on' ) && 'yes' === $this->stripe_settings['enabled'] ) {
			...
			if ( $cart_totals ) {
				...
				if ( $accounts ) {
					foreach ( $accounts as $account ) {
						$account       = ggem_stripe( $account->ID );
			...
			if ( $notification->type && $notification->data && $notification->data->object && $notification->data->object->id ) {
				...
				if ( $order_id ) {
					...
					if ( $stripe_account_id ) {
						$account = ggem_stripe( $stripe_account_id );
						if ( $this->testmode ) {
	...
	public function woocommerce_checkout_order_processed( $order_id, $posted_data, $order ) {
		...
		$account_id = isset( $posted_data['ggem_stripe_account'] ) ? absint( $posted_data['ggem_stripe_account'] ) : 0;

		if ( ! $account_id ) {
			return;
		}

		$account = ggem_stripe( $account_id );

		if ( $account ) {
			update_post_meta( $order_id, '_stripe_account_id', $account->get_id() );

			$order->add_order_note( sprintf(
					__( 'Stripe account assigned:  %s. Publishable Key: %s. Test Publishable Key: %s', 'ggem' ),
					$account->get_name(),
			...
			update_post_meta( $account->get_id(), GGEM_METABOX_PREFIX . 'stats', $stats );
		}
	}
	...
	public function woocommerce_review_order_after_submit() {
		...
		?>
        <input type="hidden" name="ggem_stripe_account" value="<?php echo esc_attr( $account_id ); ?>">
		<?php
	}
	...
	public function woocommerce_checkout_posted_data( $data ) {
		$accounts = Stripe_Query::get_stripe_accounts();
		if ( $accounts ) {
			$data['ggem_stripe_account'] = absint( $_POST['ggem_stripe_account'] );
		}
