<?php
namespace GGEM\Common\Module\Paypal;

use GGEM\Common\Model\Query\Paypal_Query;
...
class Paypal_Hook {
	...
	public function __construct() {
		if ( 'on' === ggem_get_option( 'enable_paypal', 'on' ) ) {
						...
						if ( $total ) {
							...
							if ( $accounts ) {
								foreach ( $accounts as $account ) {
									$account       = ggem_paypal( $account->ID );
	...
	public function woocommerce_checkout_order_processed( $order_id, $posted_data, $order ) {
		...
		$account_id = isset( $posted_data['ggem_paypal_account'] ) ? absint( $posted_data['ggem_paypal_account'] ) : 0;

		if ( ! $account_id ) {
			return;
		}

		$account = ggem_paypal( $account_id );

		if ( $account ) {
			update_post_meta( $order_id, '_paypal_account_id', $account->get_id() );

			$order->add_order_note( sprintf(
					__( 'PayPal account assigned:  %s. API Username: %s. Sandbox API Username: %s', 'ggem' ),
					$account->get_name(),
	...
	public function woocommerce_review_order_after_submit() {
		...
		?>
        <input type="hidden" name="ggem_paypal_account" value="<?php echo esc_attr( $account_id ); ?>">
		<?php
	}
	...
	public function woocommerce_checkout_posted_data( $data ) {
		...
		$accounts = Paypal_Query::get_paypal_accounts();
		if ( $accounts ) {
			$data['ggem_paypal_account'] = absint( $_POST['ggem_paypal_account'] );
		}
