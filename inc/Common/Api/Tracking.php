...
class Tracking extends \WC_REST_CRUD_Controller {
	 ...
	 * @var string
	 */
	protected $rest_base = 'ggem_tracking';
	...
	public function register_routes() {
		register_rest_route(
			...
			[
				'args'   => [
					'id' => [
						'description' => __( 'Unique identifier for the resource.', 'ggem' ),
						'type'        => 'integer',
					],
				],
				[
	...
	public function update_item( $request ) {
		...
		if ( ! $object || 0 === $object->get_id() ) {
			return new \WP_Error( "woocommerce_rest_{$this->post_type}_invalid_id", __( 'Invalid ID.', 'ggem' ), [ 'status' => 400 ] );
		}

		if ( ! $request->has_param( 'tracking_code' ) ) {
			return new \WP_Error( "woocommerce_rest_{$this->post_type}_missing_tracking_code", __( 'Missing Tracking ID.', 'ggem' ), [ 'status' => 400 ] );
		}
	...
	public function save_track_info_all_item( $request, $order ) {
		if ( ! class_exists( 'VI_WOO_ORDERS_TRACKING_DATA' ) ) {
			return new \WP_Error( "woocommerce_rest_{$this->post_type}_missing_tracking_plugin", __( 'Missing Tracking Plugin.', 'ggem' ), [ 'status' => 400 ] );
		}
		...
		$response                 = [
			...
			'paypal_message'           => __( '', 'ggem' ),
			'message'                  => __( 'Tracking saved', 'ggem' ),
			'detail'                   => '',
		...
		if ( $add_new_carrier ) {
			...
			if ( $carrier_name && $tracking_url && $shipping_country ) {
				...
				wp_send_json(
					[
						'status'  => 'error',
						'message' => __( 'Not enough information', 'ggem' ),
						'details' => [
							'carrier_name'     => $carrier_name,
		...
		if ( $add_to_paypal && $transID && $paypal_method && ! in_array( $tracking_number, $paypal_added_trackings ) ) {
			...
			if ( $result_add_paypal['status'] === 'error' ) {
				...
				$response['paypal_message'] = empty( $result_add_paypal['data'] ) ? __( 'Cannot add tracking to PayPal', 'ggem' ) : $result_add_paypal['data'];
			} else {
		...
		if ( ! in_array( $tracking_number, $paypal_added_trackings ) ) {
			...
			$response['paypal_button_title'] = __( 'Add this tracking number to PayPal', 'ggem' );
		} else {
			$response['paypal_button_class'] = 'inactive';
			$response['paypal_button_title'] = __( 'This tracking number was added to PayPal', 'ggem' );
		}
		...
		if ( $line_items_count = count( $line_items ) ) {
			...
			$response['message'] = __( 'Order items not matched', 'ggem' );
		}
	...
	public function add_trackinfo_to_paypal( $send_paypal, $paypal_method, $order ) {
		...
		if ( is_numeric( $i ) ) {
			...
			if ( $paypal_account_id ) {
				$account = ggem_paypal( $paypal_account_id );
				if ( $sandbox ) {
			...
			$result = [
				'status' => 'error',
				'data'   => __( 'PayPal method not found', 'ggem' ),
			];
		}
