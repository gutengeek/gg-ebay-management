...
use GGEM\Libraries\Form\Helper;
use GGEM\Libraries\Form\Field\File;
use GGEM\Libraries\Form\Field\Iconpicker;
use GGEM\Libraries\Form\Field\Map;
use GGEM\Libraries\Form\Field\Uploader;
use GGEM\Libraries\Form\Field\Taxonomy;
use GGEM\Libraries\Form\Field\Page;
...
class Form {
	...
	public function __construct() {
		// Ajax.
		add_action( 'wp_ajax_ggem_search_users', [ '\GGEM\Libraries\Form\Helper', 'ajax_search_users' ] );
	...
	public function render_field( $field ) {
		...
		if ( ! isset( $field['id'] ) || ! isset( $field['type'] ) ) {
			return sprintf( esc_html__( 'The field ID or field type is required.', 'ggem' ), $field ['type'] );
		}
		...
		switch ( $field['type'] ) {
			...
			case 'html':
				...
				break;
			default:
				do_action( 'ggem_form_render_field_' . $field['type'], $field, $this );
				//return sprintf( esc_html__( 'The field type: %s does not exist!', 'ggem' ), $field ['type'] );
				break;
		}
	}
	...
	public function ajax_user_search( $args = [] ) {
		...
		$defaults = [
			...
			'placeholder'  => esc_html__( 'Enter username', 'ggem' ),
			'label'        => null,
			'desc'         => null,
			'class'        => '',
			'disabled'     => false,
		...
		$args['class'] = 'ggem-ajax-user-search ' . $args['class'];

		$output = '<span class="ggem_user_search_wrap">';
		$output .= $this->text_field( $args );
		$output .= '<span class="ggem_user_search_results hidden"><a class="ggem-ajax-user-cancel" aria-label="' . esc_html__( 'Cancel',
				'ggem' ) . '" href="#">x</a><span></span></span>';
		$output .= '</span>';
	...
	public function text_field( $args ) {
		switch ( $args['type'] ) {
			case 'text_small' :
				$class        = 'ggem-text-small';
				$args['type'] = 'text';
				break;
			case 'text_medium' :
				$class        = 'ggem-text-medium';
				$args['type'] = 'text';
				break;
			case 'text_number' :
				$class        = 'ggem-text-small ggem-text-number';
				$args['type'] = 'number';
				break;
			case 'text_url' :
				$class        = 'ggem-text-url';
			...
			case 'text_tel' :
				...
				break;
			case 'password' :
				$class        = 'ggem-text-password';
				$args['type'] = 'password';
				break;
			case 'hidden' :
				$class        = 'ggem-text-hidden';
				$args['type'] = 'hidden';
				break;
			default :
				$class        = '';
		...
		if ( empty( $args['class'] ) ) {
			$args['class'] = 'ggem-text ' . $class . ' regular-text form-control';
		} elseif ( ! strpos( $args['class'], 'ggem-text' ) ) {
			$args['class'] .= ' ggem-text ' . $class . ' regular-text form-control';
		}
	...
	public function colorpicker_field( $args ) {
		if ( empty( $args['class'] ) ) {
			$args['class'] = 'ggem-colorpicker form-control';
		} elseif ( ! strpos( $args['class'], 'ggem-colorpicker' ) ) {
			$args['class'] .= ' ggem-colorpicker form-control';
		}
	...
	public function output_normal( $fields ) {
		...
		if ( function_exists( 'wp_unique_id' ) ) {
			$form_id = wp_unique_id( 'ggem-form-' );
		} else {
			$form_id = 'ggem-form-' . (string) ++$id_counter;
		}

		echo '<div class="js-ggem-metabox-wrap">';
	...
	public function get_repeater_field_value( $field, $field_group, $fields ) {
		 ...
		 */
		$field_value = apply_filters( "ggem_get_repeater_field_{$field['id']}_value", $field_value, $field, $field_group, $fields );
		 ...
		 * @param string $field_id
		 */
		$field_value = apply_filters( 'ggem_get_repeater_field_value', $field_value, $field, $field_group, $fields );
	...
	public function get_repeater_field_id_value( $field, $field_group, $fields ) {
		 ...
		 * @param string $field_id
		 */
		$field_value = apply_filters( "ggem_get_repeater_field_{$field['id']}_id_value", $field_value, $field, $field_group, $fields );
		 ...
		 * @param string $field_id
		 */
		$field_value = apply_filters( 'ggem_get_repeater_field_id_value', $field_value, $field, $field_group, $fields );
	...
	public function get_repeater_field_id( $field, $fields, $default = false ) {
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( "ggem_get_repeater_field_{$field['id']}_id", $field_id, $field, $fields, $default );
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( 'ggem_get_repeater_field_id', $field_id, $field, $fields, $default );
	...
	public function get_repeater_field_hidden_id( $field, $fields, $default = false ) {
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( "ggem_get_repeater_field_{$field['id']}_hidden_id", $field_id, $field, $fields, $default );
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( 'ggem_get_repeater_field_hidden_id', $field_id, $field, $fields, $default );
