...
( function( $, window ) {
    ...
    function ggem_uploader() {
        ...
        function check_number_files( file , i   ) {
            if( is_image_file(file) ) {
                if( i+1 > ggemJS.mfile_image ){
                    return false;
                }
            } else {
                if( i+1 > ggemJS.mfile_other ){
                    return false;
                }
            }
            return true;
        ...
        function check_filesize (  file , i  ) {
            ...
            if( is_image_file(file) ) {
                if( file.size > ggemJS.size_image ){
                    var myToast = $.toast({
                        heading: file.name,
                        text: ggemJS.error_upload_size,
                        icon: 'error',
        ...
        function trigger_button_upload(){
            ...
            $(".ggem-uploader-files").each( function(){
                handleUpload( this )
            } );
    ...
    /** @namespace wp */
    window.wp = window.wp || {};
    window.wp.ggem_uploader = ggem_uploader();
