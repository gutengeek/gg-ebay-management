...
( function ( window, document, $, opal, undefined ) {
    ...
    opal.init = function () {
        ...
        // Init tab form.
        opal.initTabForm( $( '.ggem-form-data-tabs' ) );

        // Make File List drag/drop sortable:
        opal.makeListSortable();

        // Init time/date/color pickers
        opal.initPickers( $metabox.find( 'input[type="text"].ggem-colorpicker' ),
            $metabox.find( 'input[type="text"].ggem-datepicker' ) );

        $metabox
            .on( 'change', '.ggem_upload_file', function () {
                opal.media.field = $( this ).attr( 'id' );
                $id( opal.media.field + '_id' ).val( '' );
            } )
            // Media/file management
            .on( 'click', '.ggem-upload-button', opal.handleMedia )
            .on( 'click', '.ggem-attach-list li, .ggem-media-status .img-status img, .ggem-media-status' +
                ' .file-status >' +
                ' span', opal.handleFileClick )
            .on( 'click', '.ggem-remove-file-button', opal.handleRemoveMedia );

        opal.mapping();
        /////
        ///
        opal.groupFields( $(".ggem-group-field-wrap", $metabox) );
        ///
        $( '.ggem-select' ).select2();
        ///
        ...
        $metabox.find( '.ggem-iconpicker' ).each( function () {
            $( this ).fontIconPicker();
        } );
    };
    ...
    opal.initTabForm = function ( $tabs ) {
        $( $tabs ).each( function () {
            ...
            $( 'a', $tab ).click( function () {
                $( '.ggem_options_panel', $parent ).hide();
    ...
    opal.makeListSortable = function() {
        var $filelist = opal.metabox().find( '.ggem-media-status.ggem-attach-list' );
        if ( $filelist.length ) {
    ...
    opal.datePickerSetupOpts = function ( fieldOpts, options, method ) {
        ...
        options.beforeShow = function ( input, inst ) {
            ...
            $id( 'ui-datepicker-div' ).addClass( 'ggem-element' );
        ...
        options.onClose = function ( dateText, inst ) {
            ...
            var $picker = $id( 'ui-datepicker-div' ).removeClass( 'ggem-element' ).hide();
    ...
    opal.handleMedia = function ( evt ) {
        ...
        var $el = $( this );
        opal.attach_id = !$el.hasClass( 'ggem-upload-list' ) ? $el.closest( '.ggem-field-wrap' )
                                                                     .find( '.ggem-upload-file-id' )
                                                                     .val() : false;
        // Clean up default 0 value
        opal.attach_id = '0' !== opal.attach_id ? opal.attach_id : false;

        opal._handleMedia( $el.prev( 'input.ggem-upload-file' ).attr( 'id' ),
            $el.hasClass( 'ggem-upload-list' ) );
    };
    ...
    opal.handleFileClick = function ( evt ) {
        ...
        evt.preventDefault();
        var $el = $( this );
        var $td = $el.closest( '.ggem-field-wrap' );
        var isList = $td.find( '.ggem-upload-button' ).hasClass( 'ggem-upload-list' );
        opal.attach_id = isList ? $el.find( 'input[type="hidden"]' ).data( 'id' ) : $td.find(
            '.ggem-upload-file-id' ).val();

        if ( opal.attach_id ) {
            opal._handleMedia( $td.find( 'input.ggem-upload-file' ).attr( 'id' ), isList, opal.attach_id );
        }
    };
        ...
        handlers.list = function ( selection, returnIt ) {
            ...
            if ( !handlers.list.templates ) {
                handlers.list.templates = {
                    image: wp.template( 'ggem-list-image' ),
                    file: wp.template( 'ggem-list-file' ),
                };
            }
            ...
            if ( !returnIt ) {
                // Append each item from our fileGroup array to .ggem-media-status
                media.$field.siblings( '.ggem-media-status' ).append( fileGroup );
            } else {
                return fileGroup;
            }
        ...
        handlers.single = function ( selection ) {
            if ( !handlers.single.templates ) {
                handlers.single.templates = {
                    image: wp.template( 'ggem-single-image' ),
                    file: wp.template( 'ggem-single-file' ),
                };
            }
            ...
            media.$field.siblings( '.ggem-media-status' ).slideDown().html( attachmentHtml );
        };
    ...
    opal.handleRemoveMedia = function ( evt ) {
        evt.preventDefault();
        var $this = $( this );
        if ( $this.is( '.ggem-attach-list .ggem-remove-file-button' ) ) {
            $this.parents( '.ggem-media-item' ).remove();
            return false;
        }
        ...
        $this.parents( '.ggem-media-status' ).html( '' );
    ...
    opal.loadAutosugesstion = function () {
       ...
       load_select2_member( '#post_author_override', 'ggem_search_users' );
    };
    opal.groupFields = function ( $group_containers ) {
        ...
        $group_containers.each( function() {
            var $_this = $( this );
            var $template = $( ".ggem-template", this );
            ...
            $(".add-repeat-group-btn", $_this ).on( "click" , function() {
                ...
                changeIndexes( $_this );
                // $( this ).closest( 'table' ).find('tbody tr:last-child select.ggem-select').select2();
            } );
            ...
            $(  $_this ).on( 'click', ".opal-row-repeater button" , function() {
             ...
             $(  $_this ).on( 'click', ".opal-row-repeater .ggem-remove" , function() {
                var group = $( this ).parent().parent().parent().parent();
                $( group ).remove();
                changeIndexes( $_this );
            } );
    ...
    opal.mapping = function() {
        ...
        var condition_cache = '';
        $('#ggem-add-new-condition').on('click', function (e) {
            e.preventDefault();
            var $filter_body = $('#ggem-table-filter tbody');
            ...
            if (condition_cache) {
                ...
                $.ajax({
                    ...
                    method: 'POST',
                    data: {
                        action: 'ggem_add_new_filter_condition'
                    }
        ...
        $(document).on('click', '.ggem-del-condition', function (e) {
            e.preventDefault();
        ...
        function condition_sortable_init() {
            var $table_body = $('.ggem-table-filter tbody');
        ...
        function no_conditions() {
            var no_conditions = $('#ggem-table-filter tbody tr:not(.ggem-no-conditions)').length;

            if (no_conditions >= 1) {
                $('.ggem-no-conditions').hide();
            } else {
                $('.ggem-no-conditions').show();
            }
        }

        function select2_mapping_init() {
            $( '.ggem-table-filter select' ).select2();
        }
    }
    ...
    opal.metabox = function () {
        ...
        opal.$metabox = $( '.js-ggem-metabox-wrap' );
        return opal.$metabox;
    };
