 ...
 * @version    $Id$
 * @package    ggem
 * @author     Opal  Team <info@ggem.com >
 * @copyright  Copyright (C) 2019 ggem.com. All Rights Reserved.
 * @license    GNU/GPL v2 or later http://www.gnu.org/licenses/gpl-2.0.html
 *
 * @website  http://www.ggem.com
 * @support  http://www.ggem.com/support/forum.html
 */
...
class Uploader {
	...
	public function render() {
		...
		$this->form->add_dependencies( 'ggem-uploader-js' );
		...
		if ( isset( $args['accept'] ) && $args['accept'] ) {
			...
			$info = [
				'size'   => ggem_get_option( 'upload_other_max_size', 0.5 ),
				'number' => ggem_get_option( 'upload_other_max_files', 10 ),
			];
			...
			$info = [
				'size'   => ggem_get_option( 'upload_image_max_size', 0.5 ),
				'number' => ggem_get_option( 'upload_image_max_files', 10 ),
			];
		}
		if ( $single ) {
			$info['number'] = 1;
		...
		?>
        <div class="ggem-uploader-files <?php echo $class; ?>" data-name="<?php echo $args['id']; ?>" data-single="<?php echo $single; ?>" data-show-icon="<?php echo $show_icon; ?>">
        ...
        <p class="ggem-metabox-description">
            <i>
				<?php
				echo sprintf( __( 'Allow upload file have size < %s MB and maximum number of files: %s', 'ggem' ),
