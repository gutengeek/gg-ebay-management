...
?>
<div class="ggem-group-field-wrap ggem-repeatable-field-section <?php echo esc_attr( $wrapper_class ); ?>" data-field="<?php echo "{$args['id']}"; ?>" id="<?php echo "{$args['id']}_field"; ?>"
	...
	<?php if ( ! empty( $args['name'] ) ) : ?>
        <p class="ggem-repeater-field-name"><?php echo trim( $args['name'] ); ?></p>
	<?php endif; ?>

	<?php if ( ! empty( $args['description'] ) ) : ?>
        <p class="ggem-repeater-field-description"><?php echo esc_html( $args['description'] ); ?></p>
	<?php endif; ?>

    <table class="ggem-repeatable-fields-section-wrapper" cellspacing="0">
		<?php
		$repeater_field_values = $this->get_field_value( $args );

		$header_title = isset( $args['options']['header_title'] )
			? $args['options']['header_title']
			: esc_attr__( 'Group', 'ggem' );
        ...
        <tr class="ggem-template">
            <td class="ggem-repeater-field-wrap ggem-column" colspan="2">
                <div class="opal-row-head ggem-move">
                    ...
                    <div class="button-action-group">
                    	<span class="ggem-remove button-action" title="<?php esc_attr_e( 'Remove Group', 'ggem' ); ?>">
        ...
        <tr>
			...
			$add_row_btn_title = isset( $args['options']['add_button'] )
				...
				: esc_html__( 'Add Row', 'ggem' );
			?>
            <td colspan="2" class="ggem-add-repeater-field-section-row-wrap">
