...
class Taxonomy {
	...
	private function render_check_item( $args, $term ) {
		...
		$output .= '<label class="ggem-option-label" for="' . $_id . '">' . esc_html( $term->name ) . '</label>';
		$output .= '</span>';
	...
	public function render_custom_style( $ismulti = true ) {
		...
		$output = '<div class="ggem-field-wrap ggem-taxonomy-select-wrap form-group" id="' . sanitize_key( $this->form->form_id . $args['id'] ) . '-wrap" >';
		if ( $args['show_label'] ) {
			$output .= '<label class="ggem-label" for="' . esc_attr( sanitize_key( str_replace( '-', '_', $this->form->form_id . $args['id'] ) ) ) . '">' . esc_html( $args['name'] ) . '</label>';
		}
	...
	public function render_taxonomy_select() {
		...
		if ( ! empty( $args['description'] ) ) {
			$output .= '<p class="ggem-description">' . esc_html( $args['description'] ) . '</p>';
		}
	...
	public function render_taxonomy_multicheck() {
		...
		$output = '<div class="ggem-field-wrap ggem-taxonomy-multicheck-wrap form-group" id="' . sanitize_key( $this->form->form_id . $args['id'] ) . '-wrap" >';
		if ( $args['show_label'] ) {
			$output .= '<label class="ggem-label" for="' . esc_attr( sanitize_key( str_replace( '-', '_', $this->form->form_id . $args['id'] ) ) ) . '">' . esc_html( $args['name'] ) . '</label>';
		}
	...
	public function render_taxonomy_radio() {
		...
		$output = '<div class="ggem-field-wrap ggem-taxonomy-radio-wrap form-group" id="' . sanitize_key( $this->form->form_id . $args['id'] ) . '-wrap" >';
		if ( $args['show_label'] ) {
			$output .= '<label class="ggem-label" for="' . esc_attr( sanitize_key( str_replace( '-', '_', $this->form->form_id . $args['id'] ) ) ) . '">' . esc_html( $args['name'] ) . '</label>';
		}
	...
	protected function no_terms_result( $error, $tag = 'li' ) {
		if ( is_wp_error( $error ) ) {
			...
			$message = esc_html__( 'No terms', 'ggem' );
			$data    = '';
		}
